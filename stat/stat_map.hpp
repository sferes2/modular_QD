...
SFERES_STAT(Map, Stat)
{
    ...
    void refresh(const E& ea)
    {
        ...
        {
	  assert(ea.container().archive().shape()[i] == behav_shape[i]);  //ea.behav_shape[i]);
	  behav_strides[i] = ea.container().archive().strides()[i];
	  behav_indexbase[i] = ea.container().archive().index_bases()[i];
        }
        ...
        for(const phen_t* i = ea.container().archive().data(); i < (ea.container().archive().data() + ea.container().archive().num_elements()); ++i)
        {
            phen_t p = *i;
        ...
        {
	  _write_archive(ea.container().archive(), std::string("archive_"), ea);
	  //	  _write_progress(ea.container().archive(),std::string("progress_archive"), ea);
#ifdef MAP_WRITE_PARENTS
	  _write_parents(ea.container().archive(), ea.container().parents(), std::string("parents_"), ea);
#endif
        }
    }
                        ...
                        const EA& ea) const
    {
        ...
        for(const phen_t* i = array.data(); i < (array.data() + array.num_elements()); ++i)
        {
            ...
            if (*i)
            {
	      behav_index_t idx = ea.container().get_index(*i);
	      //behav_index_t idx = ea.container().getindexarray(array, i);
                        ...
                        const EA& ea) const
    {
        ...
        for(const phen_t* i = array.data(); i < (array.data() + array.num_elements()); ++i)
        {
            ...
            if (*i)
            {
	      //behav_index_t posinarray = ea.container().getindexarray(array, i);
	      behav_index_t posinarray = ea.container().get_index(*i);
