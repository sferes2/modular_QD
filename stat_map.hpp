...
SFERES_STAT(Map, Stat)
{
    ...
    void refresh(const E& ea)
    {
        ...
        {
	  assert(ea.aggreg().archive().shape()[i] == behav_shape[i]);  //ea.behav_shape[i]);
	  behav_strides[i] = ea.aggreg().archive().strides()[i];
	  behav_indexbase[i] = ea.aggreg().archive().index_bases()[i];
        }
        ...
        for(const phen_t* i = ea.aggreg().archive().data(); i < (ea.aggreg().archive().data() + ea.aggreg().archive().num_elements()); ++i)
        {
            phen_t p = *i;
